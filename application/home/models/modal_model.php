...
class modal_model extends MY_Model
{
	...
	public function save($data)
	{
		...
		$modalData = array("description"=>$data["description"],"attachment"=>$data["attachment"],"attachmentSize"=>$data["attachmentSize"],
						   ...
						   "shownType"=>$data["shownType"],"isfrontmodal"=>1,"ispassed"=>0,"shownVedio"=>$data["shownVedio"],
                           "shownDescription"=>$data["shownDescription"],"introducation"=>$data["introducation"]
		);
		...
		{		
			...
			$modalBaseData["author"]=MyAuth::getCurrentUser()->email;
			$modalBaseData["operatorUserName"]=MyAuth::getCurrentUser()->email;
	...
	{	
		$sql=
		"SELECT
			  a.id,a.name,a.keyword,a.isPublished,a.publishedDateFrom,a.publishedDateTo,a.lastUpdatedTime,b.introducation,
	     ...
	     where isdeleted=0 and isfrontmodal=1 and a.id = ?";
	...
	public function delete($userId,$d)
	{
		$this->db->trans_begin();
